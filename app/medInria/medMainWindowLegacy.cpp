...
//--------------------------------------------------------------------------
// medMainWindowLegacyStyle

/*class medMainWindowLegacyStyle : public QStyle
{
public:
...
medMainWindowLegacy::medMainWindowLegacy ( QWidget *parent ) : QMainWindow ( parent ), d ( new medMainWindowLegacyPrivate )
{
...
medMainWindowLegacy::~medMainWindowLegacy()
{
    delete d;
    d = NULL;
}

void medMainWindowLegacy::mousePressEvent ( QMouseEvent* event )
{
...
void medMainWindowLegacy::restoreSettings()
{
    medSettingsManager * mnger = medSettingsManager::instance();

    this->restoreState(mnger->value("medMainWindowLegacy", "state").toByteArray());
    this->restoreGeometry(mnger->value("medMainWindowLegacy", "geometry").toByteArray());
}

void medMainWindowLegacy::saveSettings() {
    if(!this->isFullScreen())
    {
        medSettingsManager * mnger = medSettingsManager::instance();
        mnger->setValue("medMainWindowLegacy", "state", this->saveState());
        mnger->setValue("medMainWindowLegacy", "geometry", this->saveGeometry());
    }
}
 ...
 */
void medMainWindowLegacy::processNewInstanceMessage(const QString& message)
{
    if (message.toLower().startsWith("/open "))
    {
        const QString filename = message.mid(6);
        this->setStartup(medMainWindowLegacy::WorkSpace, QStringList() << filename);
    }
}

void medMainWindowLegacy::setStartup(const AreaType areaIndex,const QStringList& filenames) {
...
void medMainWindowLegacy::switchToArea(const AreaType areaIndex)
{
    switch (areaIndex)
    {
    case medMainWindowLegacy::HomePage:
        this->switchToHomepageArea();
        break;

    case medMainWindowLegacy::Browser:
        this->switchToBrowserArea();
        break;

    case medMainWindowLegacy::WorkSpace:
...
void medMainWindowLegacy::open_waitForImportedSignal(medDataIndex index, QUuid uuid)
{
...
void medMainWindowLegacy::resizeEvent ( QResizeEvent* event )
{
...
void medMainWindowLegacy::setWallScreen (const bool full )
{
    if ( full )
    {
        this->move ( 0, -30 );
...
void medMainWindowLegacy::setFullScreen (const bool full)
{
    if ( full )
        this->showFullScreen();
    else
...
void medMainWindowLegacy::toggleFullScreen()
{
    if ( !this->isFullScreen())
        this->showFullScreen();
    else
...
void medMainWindowLegacy::captureScreenshot()
{
...
void medMainWindowLegacy::showFullScreen()
{
...
void medMainWindowLegacy::showNormal()
{
    d->fullscreenButton->blockSignals(true);
    d->fullscreenButton->setChecked(false);
    d->fullscreenButton->blockSignals(false);
...
void medMainWindowLegacy::showMaximized()
{
    d->fullscreenButton->blockSignals(true);
    d->fullscreenButton->setChecked(false);
    d->fullscreenButton->blockSignals(false);
...
QWidget* medMainWindowLegacy::currentArea() const
{
    return d->currentArea;
}

void medMainWindowLegacy::switchToHomepageArea()
{
...
void medMainWindowLegacy::switchToBrowserArea()
{
...
void medMainWindowLegacy::switchToWorkspaceArea()
{
...
void medMainWindowLegacy::showWorkspace(QString workspace)
{
    d->quickAccessButton->setMinimumWidth(170);
...
void medMainWindowLegacy::showArea(QString const& area)
{
 ...
 */
void medMainWindowLegacy::toggleQuickAccessVisibility()
{
 ...
 */
void medMainWindowLegacy::hideQuickAccess()
{
    d->quickAccessWidget->hide();
 ...
 */
void medMainWindowLegacy::showShortcutAccess()
{
    if (d->shortcutAccessVisible)
    {
 ...
 */
void medMainWindowLegacy::hideShortcutAccess()
{
    if (!d->shortcutAccessVisible)
        return;
...
int medMainWindowLegacy::saveModified( void )
{
    QList<medDataIndex> indexes = medDatabaseNonPersistentController::instance()->availableItems();
...
void medMainWindowLegacy::availableSpaceOnStatusBar()
{
...
void medMainWindowLegacy::closeEvent(QCloseEvent *event)
{
...
bool medMainWindowLegacy::event(QEvent * e)
{
    switch(e->type())
    {
...
void medMainWindowLegacy::adjustContainersSize()
{
