...
medAbstractWorkspace::medAbstractWorkspace(QWidget *parent) : QObject(parent),
    d(new medAbstractWorkspacePrivate)
{
    d->parent = parent;
...
QWidget* medAbstractWorkspace::centralWidget() const
{
    QWidget *centralWidget = new QWidget;

    QSplitter *splitter = new QSplitter;
    splitter->setObjectName("medWorkspaceAreaSplitter");
    splitter->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
    splitter->setHandleWidth(2);

    // Setting up toolbox container
    medToolBoxContainer* toolBoxContainer = new medToolBoxContainer;
    toolBoxContainer->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Expanding);
    toolBoxContainer->setMinimumWidth(385);
    toolBoxContainer->addToolBox(workspace->selectionToolBox());
    for(medToolBox *tb : this->toolBoxes())
    {
        toolBoxContainer->addToolBox(tb);
    }
    ...
    viewContainerLayout->addWidget(this->tabbedViewContainers());

    // Setting up navigator
    QWidget *navigatorContainer = new QWidget;
    navigatorContainer->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Expanding);
    navigatorContainer->setMinimumWidth(186);
    navigatorContainer->setMaximumWidth(320);
    navigatorContainer->setContentsMargins(0,0,0,0);
    ...
    splitter->addWidget(navigatorContainer);
    ...
    databaseViewLayout->addWidget(medDatasourceManager::instance()->databaseDatasource()->compactViewWidget());
    d->navigatorContainer->setLayout(databaseViewLayout);

    dataSource->compactViewWidget()->resize(medDatasourceManager::instance()->databaseDatasource()->compactViewWidget()->width(),medDatasourceManager::instance()->databaseDatasource()->compactViewWidget()->height());
    //little tricks to force to recompute the stylesheet.
    dataSource->compactViewWidget()->setStyleSheet("/* */");

    connect(medDatasourceManager::instance()->databaseDatasource()->compactViewWidget(), SIGNAL(open(const medDataIndex&)),
...
QWidget *medAbstractWorkspace::statusBarWidget() const
{
